...
Miscarriage and reproductive loss affect one in four women, yet the emotional and informational support available is disproportionately dispersed. The experience can be incredibly isolating, and the current medical system does little to bridge the gap between the clinical care it offers and the emotional support people need. Whether someone receives guidance beyond the strictly necessary physical checkup often comes down to which clinic they happened to walk into. The medical side of a loss gets addressed, but the integration of it rarely is.
...
Melissa came to Unbody Lab with an early chat prototype and a clear conviction. We brought expertise in product thinking, design, and AI infrastructure, and a shared commitment to build Sibyl in a way that earns trust in a deeply sensitive context.

Our first phase focused on making Sibyl meaningfully context-aware. Rather than wrapping a chat interface around an LLM, we built a memory layer that lets Sibyl adapt to each user over time, and a retrieval-augmented generation system grounded in a library of 60+ clinically reviewed articles across five topic areas. When a user asks a medical or emotional question, Sibyl's responses draw from that validated knowledge base. For anything requiring clinical judgment, Sibyl keeps a clear boundary: it refers users to their care provider and surfaces crisis resources when needed.
...
Across both phases, Unbody owned the product architecture, AI memory and retrieval systems, UX design, and privacy-first implementation.

## Design and research, not just infrastructure

We were not handed a design spec. From the beginning, design, product thinking, and research were inseparable. We did our own research and received input from clinicians to understand what they offer patients navigating loss: which exercises have a clinically proven track record, which questions almost every woman seems to have, and which parts of the experience are usually left unspoken. That research shaped the features we considered most supportive: journaling sessions, guided exercises, peer stories, and a growing library of expert-reviewed material validated by Sibyl's clinical advisory board.
...
Privacy was never an afterthought. The data Sibyl handles is among the most personal imaginable. We built GDPR-compliant architecture from day one, ensuring user data is protected, even when evaluating the AI's performance. Sibyl was built with safety guardrails from the beginning, including logic tuned to distinguish past harm from present intent and a chat lock feature aligned with legal advice and emerging safety guidance for mental health companions.
...
<div class="project-recognition">
  <span>Featured in FemTech Insider</span>
  <span>Springboard Enterprises Women's Health Accelerator 2026</span>
  <span>Clinical guidance: Dr. Shai Mahesh, Dr. Heather Smith, Dr. Jeanette Mayse, ABPP</span>
</div>
