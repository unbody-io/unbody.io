...
## Getting Started
Before integrating Unbody's features, ensure you have created an account on Unbody.io, set up a [new project](/docs/get-started/step-1-setup), and [generated](/docs/get-started/step-2-building) API keys.
...
Semantic search leverages advanced techniques to understand the intent behind a user's query, delivering more relevant and contextual search results. For instance, if your database contains information about different types of vehicles, a user searching for "fuel-efficient cars" can easily find articles about "hybrid vehicles" and "electric cars," even if those specific terms are not mentioned in the query. This capability enhances the search experience by connecting users to related content based on broader concepts rather than relying solely on exact keyword matches.
