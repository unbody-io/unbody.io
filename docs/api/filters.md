...
## Basic Structure of a GraphQL Query[](#basic-structure-of-a-graphql-query)
...
## GraphQL Filter Operators[](#graphql-filter-operators)
...
-   **Filtering Based on a Single Field** Given below is an example query that retrieves documents with the `wordCount` greater than 1000.
...
-   **Combining Conditions with** `And` You can combine two conditions using `And`. The example given below retrieves documents with a `wordCount` greater than 1000 and a `creationDate` before January 1st, 2023.
...
-   **Combining Conditions with** `Or` Similar to `And` you can combine conditions using `Or` as well. The query given below retrieves documents with either a `wordCount` greater than 1000 or a title that contains the word “report”.
...
-   **Nested Filtering** The query given retrieves documents with a `wordCount` greater than 1000 and either a title containing “report” or shared with more than 5 users.
...
[VideoFile](/docs/api/data-types/videofile "VideoFile")[Pagination](/docs/api/pagination "Pagination")